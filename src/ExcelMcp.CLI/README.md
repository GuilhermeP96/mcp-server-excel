...
The CLI provides 32 feature command categories with 331 operations matching the MCP Server, plus `session`, `service`, and `batch` commands — the same capabilities without loading 32 tool schemas into context.
...
ExcelMcp.CLI provides **331 operations** across 32 feature command categories including Power Query, Python in Excel, Data Model/DAX, asynchronous Data Model jobs, What-If Analysis, PivotTables, Excel Tables, Charts, Drawings, VBA, Ranges, Worksheets, Workbooks, QueryTables, XML Maps, Connections, and Window Management.
